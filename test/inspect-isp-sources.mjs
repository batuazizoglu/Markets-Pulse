...
const summary={slug:s.slug,provider:s.provider,company_ids:s.company_ids,status:r.status,error:r.error,count:r.products.length,
 campaigns:r.meta?.campaigns?.map(({name,availability,expires_at})=>({name,availability,expires_at})),
...
console.log('ISP_TOTAL '+JSON.stringify({sources:results.length,healthy_sources:results.filter(x=>x.status==='ok').length,
...
products:results.reduce((a,x)=>a+x.count,0),priced:results.reduce((a,x)=>a+x.priced,0),campaigns:results.reduce((a,x)=>a+(x.campaigns?.length||0),0)}));
