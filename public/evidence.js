(() => {
  ...
  root.innerHTML = `
    ...
    <form class="ev-filters" id="evFilters">
      <div class="ev-filter-top"><label class="ev-search-label" for="evFilter-q">Paket veya kaynak ara<div class="ev-search">${svg('search')}<input id="evFilter-q" type="search" maxlength="160" placeholder="Örn. Havalimanı, Super Red veya kayıt no…" autocomplete="off"></div></label><div class="ev-periods" aria-label="Hızlı tarih seçimi"><button type="button" data-period="7">7 gün</button><button type="button" data-period="30">30 gün</button><button type="button" data-period="all" class="active">Tüm arşiv</button></div></div>
      <div class="ev-filter-grid"><label>Kaynak<select id="evFilter-source"><option value="">Tüm kaynaklar</option></select></label><label>Başlangıç<input id="evFilter-from" type="date"></label><label>Bitiş<input id="evFilter-to" type="date"></label><label>Kayıt türü<select id="evFilter-kind"><option value="">Tüm kayıtlar</option><option value="change">Değişiklik</option><option value="daily">Günlük kayıt</option><option value="baseline">İlk kayıt</option></select></label><label>Dosya durumu<select id="evFilter-availability"><option value="">Tüm durumlar</option><option value="complete">Tüm dosyalar hazır</option><option value="missing">Eksik dosyası var</option><option value="visual">Görseli var</option><option value="no_visual">Görseli yok</option></select></label><div class="ev-filter-actions"><button class="ev-button ev-primary" type="submit">Uygula</button><button class="ev-reset" type="button" id="evReset">Sıfırla</button></div></div>
    </form>
    <div class="ev-selection" id="evSelection" hidden><div><b id="evSelectedCount"></b><span>Farklı sayfalardan en fazla 10 kayıt seçebilirsiniz.</span></div><div><button class="ev-button ev-primary" id="evExport">${svg('download')} Seçilenleri ZIP indir</button><button class="ev-reset" id="evClearSelection">Seçimi temizle</button></div></div>
    ...
    <p class="ev-footnote">İlk taramada, paket değişikliğinde ve yaklaşık 24 saatte bir kanıt alınır. Eksik dosyalar ayrı işaretlenir; kayıtların tümünde dört dosya bulunmayabilir.</p>`;
  ...
  async function json(path,signal){const res=await fetch(path,{signal,cache:'no-store'});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||'Kayıtlar alınamadı. Yeniden deneyin.');return data;}
  ...
  function renderStats(summary={}){
    $('evStats').innerHTML=[['Arşiv kaydı',summary.total,'Seçili filtrelerde'],['Değişiklik kaydı',summary.changed,'Paket hareketi yakalanan'],['Eksiksiz kayıt',summary.complete,'Paket PNG · tam PNG · HTML · JSON'],['Eksik dosyalı',summary.missing,'Dosya durumundan filtreleyin']].map(([label,value,hint],i)=>`<div class="ev-stat ${i===3&&value?'ev-stat-warn':''}"><span>${label}</span><strong>${value??'—'}</strong><small>${hint}</small></div>`).join('');
  }
  function selection(){
  ...
  async function download(ids,button){
    if(!ids.length||state.busy)return;state.busy=true;const label=button.innerHTML,errorTarget=dialog.open?$('evDetailMessage'):null;button.disabled=true;button.textContent='ZIP hazırlanıyor…';
  ...
  function renderDetail(){
    ...
    dialog.innerHTML=`<header class="ev-detail-header"><div><span class="ev-eyebrow">KANIT #${escape(row.id)} · ${escape(kindName(row.kind))}</span><h2 id="evDetailTitle">${escape(row.source_name)}</h2><p>${stamp(row.captured_at)} · KKTC saati · ${row.parsed_count??0} paket</p></div><button class="ev-close" id="evClose" aria-label="Kanıtı kapat">×</button></header>
    <div class="ev-detail-toolbar"><div class="ev-detail-tabs" role="tablist" aria-label="Kanıt içeriği">${[['focus','Paket görünümü',row.has_focus],['full','Tam sayfa',row.has_screenshot],['packages','Paket verileri',row.has_json],['changes','Değişiklikler ('+row.changes.length+')',true],['compare','Öncekiyle karşılaştır',!!row.previous]].map(([mode,label,enabled])=>`<button id="evTab-${mode}" role="tab" aria-selected="${state.mode===mode}" aria-controls="evDetailBody" data-mode="${mode}" ${enabled?'':'disabled'}>${label}</button>`).join('')}</div></div>
    <div class="ev-detail-body" id="evDetailBody" role="tabpanel"></div>
    <footer class="ev-detail-footer"><div class="ev-detail-files">${[['focus','Paket PNG',row.has_focus],['image','Tam PNG',row.has_screenshot],['html','Ham HTML',row.has_html],['json','JSON',row.has_json]].map(([mode,label,available])=>available?`<a href="/api/snapshots/${row.id}/${mode}?download=1" download>${label} ↓</a>`:`<span title="Bu dosya kayıtta bulunmuyor">${label} eksik</span>`).join('')}</div><div class="ev-detail-actions"><a class="ev-button" href="${escape(safeUrl(row.source_url))}" target="_blank" rel="noopener noreferrer">Canlı kaynak ↗</a><button class="ev-button" id="evCopyLink">Bağlantıyı kopyala</button><button class="ev-button ev-primary" id="evDownloadOne">${svg('download')} Kaydı ZIP indir</button></div><div id="evDetailMessage" role="status"></div></footer>`;
    $('evClose').onclick=()=>dialog.close();$('evDownloadOne').onclick=()=>download([String(row.id)],$('evDownloadOne'));
  ...
  function value(v,unit=''){return v==null||v===''?'—':escape(typeof v==='object'?JSON.stringify(v):v)+unit;}
  ...
  function renderDetailBody(){
    ...
    if(mode==='focus'||mode==='full'){
      ...
      body.innerHTML=row.changes.length?`<div class="ev-viewer-tools"><span>Bu kanıtın alındığı taramada kaydedilen değişiklikler.</span></div><div class="ev-table-wrap"><table class="ev-table"><thead><tr><th>Paket</th><th>Hareket</th><th>Alan</th><th>Önce</th><th>Sonra</th></tr></thead><tbody>${row.changes.map(change=>`<tr><td><b>${escape(change.product_name||change.old_value||change.new_value||'Paket')}</b></td><td>${escape({added:'Eklendi',removed:'Kaldırıldı',field_changed:'Güncellendi'}[change.change_type]||change.change_type)}</td><td>${escape(fields[change.field_name]||change.field_name||'—')}</td><td class="ev-before-value">${value(change.old_value)}</td><td class="ev-after-value">${value(change.new_value)}</td></tr>`).join('')}</tbody></table></div>`:'<div class="ev-empty"><h3>Bu taramada değişiklik kaydedilmedi.</h3><p>'+ (row.kind==='baseline'?'Bu kayıt, sonraki değişiklikler için başlangıç noktasıdır.':'Bu kayıt, sayfanın o tarihteki durumunu belgeler.')+'</p></div>';
    }else if(mode==='compare'){
      const previous=row.previous;
      const sharedMode=row.has_focus&&previous.has_focus?'focus':row.has_screenshot&&previous.has_screenshot?'full':null;
      body.innerHTML=`<div class="ev-viewer-tools"><span>Aynı kaynağın bir önceki kayıtlı görüntüsü${sharedMode?' · '+(sharedMode==='focus'?'Paket görünümü':'Tam sayfa'):''}. Bu kayıt bir önceki güne ait olmayabilir.</span><button class="ev-button" id="evOpenPrevious">Önceki kaydı aç</button></div><div class="ev-compare">${[[previous,'ÖNCEKİ KAYIT'],[row,'SEÇİLEN KAYIT']].map(([item,label])=>{const imageMode=sharedMode||(item.has_focus?'focus':item.has_screenshot?'full':null);return `<section><div class="ev-compare-title"><b>${label}</b><span>${stamp(item.captured_at)} · #${item.id}</span><small>${imageMode==='focus'?'Paket görünümü':imageMode==='full'?'Tam sayfa':'Görsel yok'}</small></div><div class="ev-compare-image">${!imageMode?'<div class="ev-empty">Bu kayıtta görsel bulunmuyor.</div>':`<img src="${imagePath(item,imageMode)}" alt="${label} ${escape(stamp(item.captured_at))}">`}</div></section>`;}).join('')}</div>`;$('evOpenPrevious').onclick=()=>open(previous.id);imageErrors();
    }
    body.scrollTop=0;
  }
  function imageErrors(){dialog.querySelectorAll('img').forEach(img=>img.addEventListener('error',()=>{const fallback=document.createElement('div');fallback.className='ev-empty';fallback.textContent='Görsel yüklenemedi. Diğer dosyalara alt bölümden ulaşabilir veya kaydı yeniden açabilirsiniz.';img.replaceWith(fallback);}));}
