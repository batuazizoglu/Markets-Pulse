...
export function evaluateComparableProducts(telsimRows,kktcellRows,overrideRows=[]){
  ...
  for(const t of T){const best=bestT.get(t.id),all=pairs.filter(x=>x.t.id===t.id&&x.eligible).sort((a,b)=>b.score-a.score);let engineStatus='Reject',mutual=false;
    ...
    if(o){
      effectiveStatus=o.decision==='primary'?'Primary':o.decision==='secondary'?'Secondary':'Reject';
      // Revalidate persisted decisions against today's eligible catalog; never silently substitute a new peer.
      const candidates=all.filter(x=>x.k.key===o.kktcell_product_key);
      effectiveK=o.decision==='reject'?null:(candidates.length===1?candidates[0].k:null);
      const stale=o.decision!=='reject'&&!effectiveK;
      if(stale)effectiveStatus='Review';
      override={decision:o.decision,kktcell_product_key:o.kktcell_product_key,kktcell_product_name:o.kktcell_product_name,note:o.note,updated_at:o.updated_at,updated_by:[o.first_name,o.last_name].filter(Boolean).join(' ')||o.email||null,stale};
    }
